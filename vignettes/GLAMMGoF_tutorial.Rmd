...
Now let's compare to the `bootstrap` method for the same model, which generally should be very similar for well-behaved models and reasonably sized data sets.
...
Note on method choice for random-effect models: method = "holdout" is recommended over method = "bootstrap" when the model contains random effects with nontrivial variance components. Bootstrap resampling with replacement duplicates rows within re-sampled training sets. These duplicates appear to the model as within-group replicates, systematically inflating the estimated random-effect variance in refitted training models relative to the full-data fit. When bias_adjust = "manual" is used, the correction factor is computed once from the full-data model, so it under-corrects the inflated per-refit Jensen bias under bootstrap and leaves residual negative RBIAS in the output. This issue appears to be most pronounced in datasets with few observations per random-effect group, which is common in ecological sampling designs; it is particularly acute for balanced repeated-measures designs (e.g., a fixed number of sub-samples per site or subject), where case bootstrap with replacement generates duplicated rows that could not arise under the sampling design. In such cases, cluster bootstrap (resampling entire groups) or parametric bootstrap would be more appropriate resampling schemes; GLAMMGoF currently supports neither, so method = "holdout" is the recommended choice.


The implication for Jensen bias diagnostics in `bias_precision()` is direct: when bias_adjust = "manual", the analytical correction factor exp(V/2) is computed once from the full-data model and applied uniformly across resamples. Under `method = "holdout"`, this factor matches the per-refit random-effect variance closely, because holdout sub-sampling produces unbiased variance-component estimates. Under method = "bootstrap", the same static factor under-corrects the inflated per-refit Jensen bias, leaving residual negative RBIAS in the "manual" output. The "none" result is also more negative under bootstrap than holdout for the same reason; the true Jensen bias in each refit is larger.

Holdout resampling therefore produces internally consistent behavior between `bias_adjust = "none"` and `bias_adjust = "manual"` and is the recommended method for the Jensen diagnostic workflow. Bootstrap resampling remains appropriate for GLMs and GAMs without random effects, for random-effect models with negligible variance components, and for applications where the primary interest is in overall predictive precision rather than in diagnosing marginal-mean bias. The choice between methods is orthogonal to bias_adjust and can be made on other grounds: "bootstrap" makes more efficient use of small datasets by evaluating on out-of-bag observations, while "holdout" more closely mirrors prediction to genuinely independent unseen data.

#### A worked example: the doubly-biased case

The example below simulates data from a log-normal LMM with known variance
components, then compares the naive back-transform against the correction built
from the random effect variance alone and the full correction including the
residual term.
...
The naive back-transform recovers the geometric mean and underestimates the arithmetic mean substantially. The RE-only correction closes part of the gap,
which is what makes it dangerous: the result moves in the right direction and looks plausible, but a sizable bias remains because the residual variance here is larger than the random effect variance. Only the full correction recovers the observed mean.

`bias_precision()` applies the same correction internally when
`bias_adjust = "manual"` is used with a log-transformed response, and reports
predictive performance on the original response scale:
...
One case cannot be resolved automatically: a response that was log-transformed *before* fitting and stored as its own column. The formula then gives no
indication that a transformation occurred, so `jensen_correct()` stops with an error rather than guessing. Pass `type = "lognormal"` explicitly in that situation:
