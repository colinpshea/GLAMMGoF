...
Note on method choice for random-effect models: method = "holdout" is recommended over method = "bootstrap" when the model contains random effects with nontrivial variance components. Bootstrap resampling with replacement duplicates rows within resampled training sets, and these duplicates appear to the model as within-group replicates, systematically inflating the estimated random-effect variance in refitted training models relative to the full-data fit. This issue appears to be particularly acute for ecological studies with balanced repeated-measures designs (e.g., a fixed number of sub-samples per site or subject), where bootstrap with replacement generates duplicated rows that could not arise under the sampling design. In such cases, cluster bootstrap (resampling entire groups) or parametric bootstrap would be more appropriate resampling schemes; GLAMMGoF currently supports neither, so method = "holdout" is the recommended choice.
