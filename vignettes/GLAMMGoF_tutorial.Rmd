...
*before* fitting and stored as its own column (e.g. `logy ~ x`). The formula gives
no indication that a transformation occurred, so `jensen_correct()` stops with an
error rather than guessing. Pass `type = "lognormal"` explicitly in that situation:

```{r, warning=FALSE, comment=NA, eval=FALSE}
countData$log_y <- log(countData$y)
m_precomputed   <- lm(log_y ~ Temp, data = countData)
