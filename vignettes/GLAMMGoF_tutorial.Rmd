...
1.  Be sure you're using it with the correct model type, which in this case is one with a continuous or integer response variable. Here, a variety of distributions (e.g., Gaussian, Poisson, negative binomial, Tweedie, and gamma) are supported, as are numerous R packages (`glmmTMB`, `lme4` (including `glmer`, `glmer.nb`, and `lmer`), `MASS` (`glm.nb`), `mgcv` (`gam`), and `glm`/`lm` from the `stats` package). The function will throw an informative error message if the wrong type of model is supplied. 
...
This bias is directly analogous to the well-known log-normal back-transformation
bias. The math is identical; the only difference is which variance components sit
on the log scale, *inside* the back-transformation:
...
In the log-normal LM case, the log-scale variance is the residual variance and the
correction is widely known. In the log-link GLMM case, the log-scale variance is
the random effect variance and the same correction applies; however, this
connection seems to be rarely made explicit in ecological statistics training.

The third column is where the two meet. A log-normal model *with* random effects
is subject to both sources of bias simultaneously, because both the residual error
and the random effects are additive on the log scale and therefore both sit inside
the exponential. The correction is
$\exp((\sigma^2_{\varepsilon} + \sigma^2_{u}) / 2)$, and omitting the residual
term is a common and consequential mistake: residual variance is typically the
*larger* of the two components, so a correction built from `VarCorr()` alone can
recover only a fraction of the bias while appearing to have addressed it. A
fixed-effects GLM with a log link has no random component on the linear predictor
and therefore no Jensen bias at all; $\exp(\beta_0)$ is exactly the arithmetic
mean.
...
**This is not a model deficiency:** Rather, it is a structural property of GLMMs. Consistent negative RBIAS in `bias_precision()` output signals that this heterogeneity is being ignored in marginal predictions. For purely inferential purposes (contrasts, p-values, effect directions) this does not matter. For calibrated response-scale predictions (abundance maps, density estimates, management outputs), the correction should be applied.
...
The single most important distinction is whether the log appears as a **link
function** or as a **transformation of the response**, because this determines
whether the residual variance sits inside or outside the back-transformation. The
family alone does not tell you: a Gaussian model can appear on either side of this
line.
...
`gaussian(link = "log")` and `log(y) ~ x` are easy to
conflate, as both involve a Gaussian likelihood and an exponential, but they are
different models. With a log link the response is untransformed and the residual
error is additive on the *response* scale, outside $\exp()$, so it contributes
nothing to the re-transformation bias. With a log-transformed response the residual
error is additive on the *log* scale, inside $\exp()$, so it contributes fully.

`GLAMMGoF` resolves this automatically by inspecting the left-hand side of the
model formula rather than the family or link, so both cases are handled correctly
without user intervention. Only a natural log is supported; `log10()`, `log2()`,
and `log1p()` responses are rejected with an informative error, since each implies
a different back-transformation.
...
The naive back-transform recovers the geometric mean and underestimates the
arithmetic mean substantially. The RE-only correction closes part of the gap,
which is what makes it dangerous: the result moves in the right direction and
looks plausible, but a sizeable bias remains because the residual variance here is
larger than the random effect variance. Only the full correction recovers the
observed mean.
...
remains incompatible with `bias_adjust = "manual"`. Note that the reasoning
differs slightly from the log-link case: conditional predictions absorb the random
effect contribution but *not* the residual contribution, since the residual is
always marginalized. A conditional log-normal prediction therefore still requires
a residual-only correction of $\exp(\sigma^2_\varepsilon/2)$, which is not
currently implemented.

One case cannot be resolved automatically: a response that was log-transformed
*before* fitting and stored as its own column. The formula then gives no
indication that a transformation occurred, so `jensen_correct()` stops with an
error rather than guessing. Pass `type = "lognormal"` explicitly in that
situation:
