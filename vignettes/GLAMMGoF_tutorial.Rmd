...
-   `Log Loss (Binary Cross-Entropy)`: Across all observations, the mean negative log-likelihood of the observed outcomes under the predicted probabilities, where lower values indicate better performance. Like the `Brier score`, `log loss` rewards calibration, but its logarithmic scale means it penalizes extreme, confident mispredictions far more aggressively. `Log loss` is therefore the most sensitive of the three to cases where the model assigns near-zero probability to an outcome that actually occurs. This function also returns the null (i.e., intercept-only) model log loss for reference (i.e., the best predictive performance achievable without any predictors), which allows for comparison to the summarized log loss metrics. A null model reference is not required for Brier and AUC, as they both have natural reference points (0 for Brier score, 0.5 for AUC).
