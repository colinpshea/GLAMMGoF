...
|:-----------------------|:-----------------------|:-----------------------|
| Model structure | $\log(Y) \sim \text{Normal}(X\beta, \sigma^2)$ | $\log(\mu_{ij}) = X\beta + u_j$, $u_j \sim \text{Normal}(0, \sigma^2)$ |
| Log-scale variance | Residual $\sigma^2$ | Random effect variance $\sigma^2$ |
| Naive back-transform | $\exp(X\beta)$ | $\exp(X\beta)$ |
| True arithmetic mean | $\exp(X\beta + \sigma^2/2)$ | $\exp(X\beta + \sigma^2/2)$ |
| Correction factor | $\exp(\sigma^2/2)$ | $\exp(\sigma^2/2)$ |

In the log-normal case, the log-scale variance is the residual variance and the correction is widely known. In the log-link GLMM case, the log-scale variance is the random effect variance and the same correction applies; however, this connection seems to be rarely made explicit in ecological statistics training. It follows that log-normal models with random effects are subject to both sources of bias simultaneously (residual variance and random effect variance), requiring a combined correction of $\exp((\sigma^2_{residual} + sigma^2_{random effect} / 2)$, where both variance components are additive on the natural log scale. A fixed-effects GLM with a log link has no random component on the linear predictor and therefore no Jensen bias; $\exp(\beta_0)$ is exactly the arithmetic mean.
