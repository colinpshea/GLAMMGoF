...
The fourth row is the trap. `gaussian(link = "log")` and `log(y) ~ x` are easy to
conflate - both involve a Gaussian likelihood and an exponential - but they are
...
nothing to the retransformation bias. With a log-transformed response the residual
...
Two limitations apply to log-normal models specifically. First,
`bias_adjust = "tmb"` is not available: TMB's bias correction integrates over the
random effects but does not include the residual retransformation term, so it
would under-correct; `bias_adjust = "manual"` is the correct choice and an
informative error is thrown otherwise. Second, `conditional_predictions = TRUE`
...
`bias_precision()` provides a `bias_adjust` argument with three options for diagnosing and correcting Jensen's inequality bias. The `"manual"` correction is available for `glmmTMB` and `lme4` models, and for `lm`/`glm` fits to a log-transformed response; `"tmb"` is available for `glmmTMB` models only, and not for models fit to a log-transformed response. A message is automatically triggered when both in-sample and out-of-sample RBIAS are consistently negative (\< -10%), prompting the user to investigate. The recommended diagnostic workflow is:
...
Note that `bias_adjust = "manual"` is not supported for binomial models in `brier_auc()`, since the analytical correction is only valid where predictions are back-transformed via the exponential - that is, for log-link and log-transformed-response models. Use `bias_adjust = "tmb"` instead for binomial GLMMs.
...
| `"tmb"` | `FALSE` or `TRUE` | Corrected population-level | Corrected population-level marginal mean via TMB's AD; uses `re.form = NULL` internally as a computational requirement, not to obtain group-specific predictions. Not available for models fit to a log-transformed response |
...
For models fit to a log-transformed response (`log(y) ~ .`), `bias_adjust = "tmb"` throws an informative error: TMB's correction integrates over the random effects but omits the residual retransformation term, and would therefore under-correct. Use `bias_adjust = "manual"`, which includes both components. The `"manual"` + `conditional_predictions = TRUE` combination remains blocked for these models as well, though for a slightly different reason - conditional predictions absorb the random effect contribution but not the residual contribution, so a residual-only correction would still be required, and this is not currently implemented. For `brier_auc()`, the same logic applies with the exception that `bias_adjust = "manual"` is not available for binomial models regardless of `conditional_predictions`.
...
For obvious reasons, the code below will cause `bias_precision` to quit and throw an error message:
...
For equally obvious reasons, the code below will cause `brier_auc` to quit and throw an error message:
