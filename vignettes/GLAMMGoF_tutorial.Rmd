...
bp$bias_precision_results <- bp$bias_precision_results %>% mutate_at(vars(value), funs(round(., 2)))
```
...
```{r, warning=FALSE, comment=NA, echo=FALSE}
bp$bias_precision_summary <- bp$bias_precision_summary %>% mutate_at(vars(mn, lwr95, upr95), funs(round(., 2)))
```
