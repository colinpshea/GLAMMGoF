...
Log loss is computed internally as:
...
For zero-heavy count or continuous data, the choice of performance metric matters. RBIAS in particular can behave unexpectedly when the response contains zeros, because the relative error formulation divides by the observed value, producing extreme terms that systematically pull the mean downward and give a misleading impression of systematic underprediction. This is a known property of relative bias metrics rather than evidence of poor model fit (Hyndman and Koehler 2006), and is most likely to manifest with Tweedie, zero-inflated, or hurdle models where zeros are structurally common.

In these situations, RMedAE is the most reliable summary of predictive performance. Because it is based on the median rather than the mean of the absolute error distribution, it is insensitive to the extreme values that zeros can introduce, and gives the best picture of accuracy for a typical observation. When combined with the DHARMa zero-inflation test (`testZI = TRUE`), which evaluates whether the model correctly captures the frequency of zeros in the data, RMedAE and the ZI test together provide a more complete assessment than any single metric alone: RMedAE characterizes how well the model predicts typical non-extreme values, while the ZI test flags whether the zero-generating process is being adequately captured. A significant ZI test result with ratioObsSim > 1 indicates the model is underpredicting zeros and a zero-inflated model family should be considered.
