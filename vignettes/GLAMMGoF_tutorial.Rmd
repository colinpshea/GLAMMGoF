...
Note on method choice for random-effect models: method = "holdout" is recommended over method = "bootstrap" when the model contains random effects with nontrivial variance components. Bootstrap resampling with replacement duplicates rows within resampled training sets, and these duplicates appear to the model as within-group replicates, systematically inflating the estimated random-effect variance in refitted training models relative to the full-data fit. This is a general property of case bootstrap in mixed models, not a limitation specific to GLAMMGoF; parametric bootstrap methods are typically preferred for variance component estimation in mixed models for the same reason.
