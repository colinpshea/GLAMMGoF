...
The `bias_adjust` and `conditional_predictions` restrictions for log-transformed-response models (no `"tmb"`; no `"manual"` with conditional predictions) are described in the worked example above. For `brier_auc()`, the same logic applies, except that `bias_adjust = "manual"` is not available for binomial models regardless of `conditional_predictions`: the analytical correction is only valid where predictions are back-transformed via the exponential (log-link and log-transformed-response models), so binomial GLMMs must use `bias_adjust = "tmb"`.
