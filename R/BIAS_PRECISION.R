  ...
  cf_internal <- if (!is.null(correction_factor) && bias_adjust == "manual") {
    ...
    if (rev_full$slopes)
      warning("Random slopes detected; bias_adjust = 'manual' uses intercept ",
              "variances only and is an approximation for random-slope models. ",
              "The true marginal correction under random slopes is per-observation ",
              "(exp(x_i^T Sigma x_i / 2)) rather than a scalar factor, and its ",
              "error grows with slope variance and with the magnitude of the ",
              "associated covariate. The approximation is reasonable when slope ",
  ...
  if (verbose && bias_adjust == "none" && jensen_plausible) {
    ...
    if (both_negative) {
      message(
        ...
        "Consider re-running with bias_adjust = 'manual' to apply a Jensen bias ",
        "correction and confirm the source of the bias. ",
