...
#' @param conditional_predictions Logical. If `TRUE` and the model is a `glmmTMB` fit with random effects, both in-sample and out-of-sample predictions are made conditionally on the estimated random effects (`re.form = NULL`, `allow.new.levels = TRUE`) rather than marginally (`re.form = ~0`). Since GLAMMGoF's holdout CV splits rows within groups rather than groups themselves, random effect estimates from the training model are valid for held-out observations from the same groups, making conditional out-of-sample predictions well-defined and meaningful. In-sample and out-of-sample metrics remain directly comparable since both use the same conditional prediction strategy; the gap between them reflects genuine overfitting to training rows rather than any marginal vs conditional distinction. The default is `FALSE`, which uses marginal predictions for both when `bias_adjust = "none"`, assessing population-level generalization. Note that `bias_adjust = "tmb"` always uses conditional predictions internally regardless of this setting, since TMB's bias correction requires random effects to be present in the linear predictor; `conditional_predictions` therefore has no additional effect when `bias_adjust = "tmb"`. This argument is silently ignored for non-`glmmTMB` models.
