...
#' @param method Character string specifying the resampling method to use. The default, `"holdout"`, repeatedly splits the data into random training and testing sets (Monte Carlo cross-validation). `"bootstrap"` samples the training data with replacement and evaluates in-sample performance on the bootstrap sample and out-of-sample performance on the out-of-bag observations not selected in the bootstrap sample (approximately 36.8% of observations on average). For random-effect models with nontrivial variance components, `"holdout"` is recommended; see the vignette for details. For models without random effects, both methods produce equivalent results up to resampling noise.
...
#'
#' Step 2 (optional) - Re-run with `conditional_predictions = TRUE` to assess within-group classification accuracy using estimated random effects. This measures how well the model interpolates within observed groups rather than generalizing to new ones.
...
brier_auc <- function(nReps = 100, testModel = NULL, testData = NULL,
  ...
  # and no bias adjustment is currently supported in brier_auc().
