...
#'
#' **Random effects and Jensen's inequality:** All predictions are population-level (i.e., random effects are set to zero via `re.form = ~0`). For models with random effects and a nonlinear link function such as the logit, backtransforming the linear predictor to the probability scale introduces a systematic bias in the predicted mean probability. This occurs because Jensen's inequality implies that `E[plogis(eta)] != plogis(E[eta])` for any random variable `eta`. The direction and magnitude of this bias depend on the curvature of the inverse-logit function at the linear predictor value, and grow with random effect variance. Consistent negative patterns in Brier score or log loss relative to the null model baseline may partly reflect this structural property of the GLMM. To obtain bias-corrected marginal predictions for `glmmTMB` models, set `bias_adjust = "tmb"`, which applies the generic bias correction of Thorson & Kristensen (2016) via TMB's automatic differentiation. Note that `bias_adjust = "manual"` is not supported for binomial models as the analytical lognormal correction is not valid for logit-link models. Bias correction is not available for `lme4` or `mgcv` models; users requiring bias correction for these model types are encouraged to refit in `glmmTMB`.
#'
#' **Random slopes:** For models with random slopes, `bias_adjust = "tmb"` is the only appropriate correction option, as TMB's automatic differentiation correctly integrates over the full random effect covariance structure including slope variance. The Jensen bias varies by observation in random slopes models as a function of the covariate associated with the slope, making a scalar correction inappropriate.
#'
#' **Correction factor and out-of-sample leakage:** For `bias_adjust = "tmb"`, predictions are conditioned on the full RE structure (`re.form = NULL`) using BLUPs estimated from each refitted training model. This means the correction implicitly incorporates information about the RE variance structure from the training data, which introduces a minor form of information leakage into out-of-sample performance metrics. As with `bias_adjust = "manual"` in `bias_precision()`, this is considered an acceptable tradeoff: the primary purpose of bias correction in this context is diagnostic confirmation that Jensen's inequality is the source of observed metric patterns, and stable corrected metrics are more informative than strictly pure but potentially unstable out-of-sample estimates.
#'
#' **Conditional predictions:** By default, both in-sample and out-of-sample predictions are made marginally (`re.form = ~0`) to assess population-level generalization. Setting `conditional_predictions = TRUE` switches both to conditional predictions (`re.form = NULL`, `allow.new.levels = TRUE`), using the random effect estimates from the refitted training model for all predictions. Since GLAMMGoF's holdout CV splits rows within groups, the training model has RE estimates for all groups in the test set. The gap between in-sample and out-of-sample metrics reflects genuine row-level overfitting rather than any marginal vs conditional distinction.
#'
#' **Recommended workflow:** The following stepwise workflow is recommended for assessing predictive performance of a glmmTMB binomial GLMM. See the package vignette for a fully worked example.
#'
#' Step 1 - Run with defaults (`bias_adjust = "none"`, `conditional_predictions = FALSE`): assess population-level predictive generalization using Brier score, AUC, and log loss relative to null model baselines.
#'
#' Step 2 (optional) - Re-run with `bias_adjust = "tmb"` to apply TMB's automatic differentiation bias correction for Jensen's inequality on the logit scale. Note: this is considerably slower than the default and is most relevant when random effect variance is large. Note that `bias_adjust = "manual"` is not available for binomial models.
#'
#' Step 3 (optional) - Re-run with `conditional_predictions = TRUE` to assess within-group classification accuracy using estimated random effects. This measures how well the model interpolates within observed groups rather than generalizing to new ones.
...
brier_auc <- function(nReps = 100, testModel = NULL, testData = NULL,
  ...
  # --- conditional_predictions message ---
  if (conditional_predictions && (is_glmmTMB || is_glmer)) {
    message(
      ...
      "assessment use the default conditional_predictions = FALSE."
    )
  }
  ...
  # --- Predict helper: unified for train and test ---
  # When conditional_predictions = TRUE both use re.form = NULL since
  # holdout CV splits rows within groups, not groups themselves.
  # When conditional_predictions = FALSE (default) both use marginal predictions.
  ...
  for (j in seq_len(nReps)) {
    ...
    results[[j]] <- data.frame(
      ...
      train_logloss = log_loss(train[[resp_var]], yhat_train),
